...
const setupPlayerByRoom = async (playerId, playerRoomId) => {
  ...
  const {roomExists, isPlayerExist, updatedSprite} = await checkIfRoomExists(playerId,playerRoomId)
  ...
  if(roomExists && isPlayerExist){
    ...
    console.log('Room exists but player is not in the room. Putting player in room')
...
const checkIfRoomExists = async (playerId, playerRoomId) => {
  ...
  const isPlayerExist = roomExists ? await checkIfPlayerExistsInRoom(playerId, document) : false;
...
const insertValueForPlayerId = async () => {
    ...
    const options = {
      arrayFilters: [
        { [`elem.${playerId}`]: { $exists: true } }
      ]
    };

    const updateResult = await collection.updateOne(filter, update, options);
    console.log(`Items pushed into the array with key "${coda}":`, updateResult.modifiedCount);
...
const updateRoomWithPlayer = async (playerId, playerRoomId, updatedRoomSprites) => {
    // Update query
    const filter = {
      roomId: playerRoomId
    };
...
const getPlayerSprites = async (playerRoomId, playerId) => {
  ...
  await collection.findOne(query)
              .then((result) => {
                if(result == null){
                  ...
                  roomExists =  true
                  console.log("The result in checking if room exists for sprites", result)
                  document = result;
                }
              }).catch((err) => {
  ...
  const playerSpritesFind = players?.find(obj => obj.hasOwnProperty(playerId));
